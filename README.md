...
| Command | Purpose |
|--|--|
| `diffprompt quickstart` | Zero-config mock demo. 5 cases, no API keys, ~10s. |
| `diffprompt init [--force] [--wizard --describe <text>] [--mock]` | Scaffold config, `prompts/`, `data/cases.jsonl`. `--wizard` asks the judge model (or a mock template) to draft prompts + 10 cases from a one-sentence description. |
| `diffprompt providers test <name>` | Hello-world smoke-test against a configured provider. Redacts auth headers. |
...
| `diffprompt seed --from-{langfuse,helicone,langsmith,openai-logs,synthetic,csv} <export>` | Convert an LLM-observability export, CSV, or synthetic template into `data/cases.jsonl` + a calibration skeleton. |
