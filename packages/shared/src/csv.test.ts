...
describe('renderCostCsv', () => {
  ...
  test('includes modelB column when compare-models is in play', () => {
    const cells: CellResult[] = [
      baseCell({ model: 'openai/gpt-5.1' as ModelId, modelB: 'openai/gpt-5.2' as ModelId }),
    ];
    const csv = renderCostCsv({ cells, summary: baseSummary });
    const lines = csv.trim().split('\n');
    expect(lines[0]).toBe('caseIndex,model,modelB,verdict,reason,error,latencyMs,costUsd');
    expect(lines[1]).toBe('0,openai/gpt-5.1,openai/gpt-5.2,b,candidate wins,,,');
  });
