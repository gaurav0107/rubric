...
- **In-UI override buttons in `rubric serve`.** Every grid cell's detail pane gets three verdict buttons (`[a]` `[b]` `[=]`), an optional reason field, and an undo button. A `✎` glyph pins on the row's index column when an override is active, and the summary strip shows a live `overrides: N` counter. Round-trips with `rubric disagree` — both writers compute the same contentKey against the same override log, so a CLI override lights up in the UI and vice-versa. Two new endpoints on the serve HTTP layer: `GET /api/overrides` (latest-wins collapse) and `POST /api/overrides` (`{caseIndex, model, modelB?, verdict, reason?, undo?}`). Closes the "CLI-only override" gap called out in the v2.2 notes — without this, the v2.3 calibration corpus doesn't get enough data to train against.
